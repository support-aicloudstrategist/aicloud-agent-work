...
# First meaningful paragraph after title = description (first 220 chars).
# Skip italicized bylines (lines starting with *) and any line < 80 chars —
# the first real paragraph tends to be long and substantive.
DESC=$(awk '
    /^# / { started=1; next }
    started && /^[A-Za-z0-9"]/ && length($0) >= 80 { print; exit }
' "$SRC" | head -c 240)
if [ -z "$DESC" ]; then
    # Fall back to the first non-blank, non-byline line
    DESC=$(awk '
        /^# / { started=1; next }
        started && NF > 0 && !/^\*[^*]+\*\s*$/ && !/^_.*_$/ { print; exit }
    ' "$SRC" | head -c 240)
fi
...
# Inject JSON-LD BlogPosting schema (idempotent — skips posts that already have it)
python3 /docker/task-queue/bin/add-schema.py || echo "schema injector failed (non-fatal)"
